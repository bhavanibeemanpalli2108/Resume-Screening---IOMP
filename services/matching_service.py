...
def _cached_fetch_resumes():
    """Fetch all resumes from DB, cached for 5 minutes."""
    resp = supabase.table("resumes").select("id, user_id, embedding, skills, experience_years, projects").execute()
    return resp.data or []


@st.cache_data(show_spinner=False, ttl=300)
def _cached_fetch_applications(job_id: str):
    """Fetch applications for a job, cached for 5 minutes."""
    resp = (
...
def compute_match(resume: dict, job: dict) -> dict:
    """
    Compute composite ATS score using FAISS for semantic similarity
    and direct scoring for skill/project/experience components.
    """
    from services.resume_service import score_projects

    try:
        # Use FAISS-accelerated semantic score when possible
        resume_emb = resume.get("embedding")
        job_emb = job.get("embedding")

        if resume_emb and job_emb:
            # Build a mini FAISS index with just this resume to get L2-normalised cosine
            index = build_faiss_index([resume_emb])
            top = faiss_top_k(index, job_emb, k=1)
            if top:
                # Re-compute exact cosine for the single match (FAISS IP == cosine after normalisation)
                sem = float(cosine_similarity(resume_emb, job_emb))
            else:
                sem = 0.0
        else:
            sem = 0.0
