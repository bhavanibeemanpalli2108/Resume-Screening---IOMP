...
def _fetch_my_applications(user_id):
    resp = supabase.table("applications").select("*").eq("candidate_id", user_id).execute()
    return resp.data or []


@st.cache_data(show_spinner=False, ttl=300)
def _fetch_job_title(job_id):
    resp = supabase.table("jobs").select("title").eq("id", job_id).execute()
    return resp.data[0]["title"] if resp.data else "Unknown Job"
    ...
    if existing.data:
        ...
        if st.button("Apply", key=f"apply_{job['id']}"):
            if not resume:
                ...
                _fetch_active_jobs.clear()
                _fetch_my_applications.clear()
...
apps_resp = _fetch_my_applications(user_id)
...
for app in applications:
    job_resp = _fetch_job_title(app["job_id"])
    job_title = job_resp
