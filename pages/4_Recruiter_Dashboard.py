...
def _fetch_applications(job_id):
    resp = (
...
def _fetch_candidate_info(candidate_id):
    """Fetch email + resume path in two queries, cached per candidate."""
    user_resp = supabase.table("users").select("email").eq("id", candidate_id).execute()
    resume_resp = supabase.table("resumes").select("file_path, extracted_text, skills").eq("user_id", candidate_id).execute()
    email = user_resp.data[0]["email"] if user_resp.data else None
...
for job in jobs:
    ...
    # Fetch applications
    applications = _fetch_applications(job["id"])
    ...
    if st.button(f"Run Auto-Shortlist", key=f"auto_{job['id']}"):
        ...
        for app in above:
            ...
            candidate_email, resume_data = _fetch_candidate_info(app["candidate_id"])
            resume_text = resume_data.get("extracted_text", "")
            resume_skills = resume_data.get("skills") or []
            job_skills = job.get("skills") or []
            missing_skills = [s for s in job_skills if s not in resume_skills]
            feedback = generate_feedback(
                resume_text=resume_text, job_title=job["title"],
                job_description=job.get("description", ""), score=app["similarity_score"],
                skills_matched=resume_skills, skills_missing=missing_skills, outcome="shortlisted",
            )
            if candidate_email:
                if not send_shortlist_email(candidate_email, job["title"], feedback=feedback):
        ...
        for app in below:
            ...
            feedback = generate_feedback(
                resume_text=resume_text, job_title=job["title"],
                job_description=job.get("description", ""), score=app["similarity_score"],
                skills_matched=resume_skills, skills_missing=missing_skills, outcome="rejected",
            )
            if candidate_email:
                if not send_rejection_email(candidate_email, job["title"], feedback=feedback):
                    failed_emails.append(candidate_email)
            rejected_count += 1
        ...
        _fetch_applications.clear()
        _fetch_candidate_info.clear()
        st.rerun()
    ...
    for app in applications:
        ...
        with col1:
            st.markdown(
                f"""<div class="role-card">
                    <h4>{candidate_email or 'Unknown'}</h4>
        ...
        with col2:
            if file_path:
                signed = supabase.storage.from_("resumes").create_signed_url(str(file_path), 120)
            ...
            if app["status"] == "applied":
                if st.button("Shortlist", key=f"shortlist_{app['id']}"):
                    supabase.table("applications").update({"status": "shortlisted"}).eq("id", app["id"]).execute()
                    if candidate_email:
                        send_shortlist_email(candidate_email, job["title"])
                    _fetch_applications.clear()
                    _fetch_candidate_info.clear()
                    st.success("Shortlisted.")
                    st.rerun()
